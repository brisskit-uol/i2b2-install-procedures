#!/bin/bash
#-----------------------------------------------------------------------------------------------
# Acquires i2b2 data and source zip files plus some ancillaries. Also major prerequisites.
...
#
# List:
#   (1) i2b2 data and source zips plus axis war file.
#   (2) Export plugin for the web client.
#   (3) i2b2 admin procedures.
#   (4) Integration Web service used to receive data from CiviCRM.
#   (5) Java JDK.
#   (6) Ant used for i2b2 build. 
...
print_message "" $LOG_FILE
print_message "Acquiring i2b2 web client's CiviCRM plugin..." $LOG_FILE
wget --user=$MVN_DEPLOY_USER \
     --password=$MVN_DEPLOY_PASSWORD \
     -O $ACQUISITIONS/$( basename $I2B2_CIVI_EXPORT_PLUGIN_DOWNLOAD_PATH ) $I2B2_CIVI_EXPORT_PLUGIN_DOWNLOAD_PATH
exit_if_bad $? "Failed to acquire i2b2 web client's CiviCRM plugin." $LOG_FILE
print_message "Success! Acquired i2b2 web client's CiviCRM plugin." $LOG_FILE
